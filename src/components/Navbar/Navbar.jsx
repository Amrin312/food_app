...
const Navbar = ({ setShowLogin }) => {
  ...
  return (
    <nav className='px-4 py-4 md:px-8 lg:px-16 md:p-5 flex justify-between items-center shadow-md bg-white sticky top-0 z-50'>
        <Link to="/"><img src={assets.logo} alt="" className='h-auto w-[90px]'/></Link>

        {/* Desktop menu  */}

        <div className='hidden md:flex gap-6'>
          <ul className='flex gap-6 font-medium'>
            <Link to='/' onClick={() => setDisplayMenu('home')} className='cursor-pointer hover:text-red-500'>Home</Link>
        ...
        {/* mobile menu */}
          <div className='flex items-center gap-2 md:hidden'>
            ...
            <button className='' onClick={() => setDisplayMenu(prev => !prev)}>
                { displayMenu ? <X /> : <Menu /> } 
            </button>
          </div>
